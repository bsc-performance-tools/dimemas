char fs_c_rcsid[]="$Id: fs.c,v 1.4 2006/09/18 13:19:48 jgonzale Exp $";
/*
 * File System null routines
 * 
...
static struct t_fh_commid *
get_fh_commid(struct t_thread *thread, int fh)
{
...
create_barrier (struct t_thread *thread)
{
...
static struct t_action* 
create_scatter(struct t_thread *thread, t_boolean twin)
{
  struct t_action *action;
  struct t_action *cur;
  struct t_fh_commid *fh_commid;
...
static void
create_fh_to_communicator (struct t_thread *thread)
{
  ...
  {
    /* Create a new one */
    fh_commid = (struct t_fh_commid *)mallocame (sizeof(struct t_fh_commid));
...
static t_micro
compute_IO_collective_time(struct t_queue *threads)
{
  register t_micro ti=0;
  int size, num_blocks;
  struct t_thread *th;
  ...
  {
    ti = file_system_parameters.disk_latency*1e6;
...
static t_micro
compute_IO_time (struct t_thread *thread)
{
  register t_micro ti=0;
...
void
Next_Action_to_thread (struct t_thread *thread)
{
   struct t_action *action;
...
void
FS_general(int value, struct t_thread *thread)
{
         ...
         switch (mpi_io->which_io)
         {
	      ...
	      cpu=get_cpu_of_thread(thread);
              Paraver_event (cpu->unique_number, IDENTIFIERS (thread),
                      current_time,
                      PARAVER_IO, PARAVER_IO_METADATA_BEGIN);
		  ...
		  cpu=get_cpu_of_thread(thread);
                  Paraver_event (cpu->unique_number, IDENTIFIERS (thread),
                          current_time,
                          PARAVER_IO, PARAVER_IO_BLOCK_NONCOLLECTIVE_READ_BEGIN);
                  copy = duplicate_thread_fs(thread);
...
A:
           ...
           case MPI_IO_NonBlock_NonCollective_Begin:
              ...
              switch (mpi_io->Oop)
              {
                ...
                case 16:
                  copy = duplicate_thread_fs(thread);
                  copy->last_paraver = current_time;
                  copy->action = (struct t_action *)mallocame(sizeof(struct t_action));
		  ...
		  cpu=get_cpu_of_thread(thread);
                ...
                case 18:
                case 19:
                  request_thread = (struct t_request_thread *)mallocame(sizeof(struct t_request_thread));
...
B:                
		...
		cpu = get_cpu_of_thread(copy);
                Paraver_thread_IO (cpu->unique_number, 
                        copy->task->Ptask->Ptaskid,
                        copy->task->taskid,
                        count_queue(&copy->task->threads)+1,
           ...
           case MPI_IO_NonBlock_NonCollective_End:
                      ...
                      thread, mpi_io->request))!=(struct t_request_thread *)0)
              {
                ...
                freeame ((char *)request_thread, sizeof(struct t_request_thread *));
		cpu=get_cpu_of_thread(thread);
                Paraver_event (cpu->unique_number, IDENTIFIERS (thread),
                     current_time,
		  ...
		  cpu=get_cpu_of_thread(thread);
              ...
              copy = duplicate_thread_fs(thread);
              copy->task->io_thread = TRUE;
              copy->threadid = count_queue(&copy->task->threads)+1;
              action = create_barrier (thread);
              fh_commid = get_fh_commid (thread, mpi_io->fh);
              action->desc.global_op.comm_id = fh_commid->communicator->communicator_id;
              fh_commid->stage  = IN_BARRIER;
      ...
      case IO_Block_Collective:
         ...
         switch (fh_commid->stage)
         {
		  ...
		  cpu=get_cpu_of_thread(thread);
               ...
               break;
            default:
                panic ("Unkown stage %d for blocking and collective IO\n",
                        fh_commid->stage);
          }
         break;
	 ...
	 cpu = get_cpu_of_thread(copy);
         Paraver_thread_IO (cpu->unique_number, IDENTIFIERS (copy),
                 copy->last_paraver, current_time);
         thread->last_paraver = current_time; 
         delete_duplicate_thread_fs(copy);         
         Next_Action_to_thread (thread);
      ...
      case IO_NonBlock_NonCollective:
	    ...
	    cpu=get_cpu_of_thread(thread);
            Paraver_event (cpu->unique_number, IDENTIFIERS (thread),
                    current_time,
                    PARAVER_IO, PARAVER_IO_NONBLOCK_NONCOLLECTIVE_END);
            freeame ((char *)copy->action, sizeof(struct t_action));
            Paraver_thread_block_IO (0, IDENTIFIERS(thread),
                    thread->last_paraver, current_time);
            thread->last_paraver = current_time;
            action = thread->action;
         ...
         switch (fh_commid->stage)
         {
		    ...
		    cpu = get_cpu_of_thread (thread);
                    Paraver_thread_IO (cpu->unique_number,
                            ...
                            thread->task->taskid,
                            count_queue(&thread->task->threads)+1,
                            current_time, new_time);
                    thread->last_paraver = new_time;
                  }
                  fh_commid->stage = IN_IO;
...
free_disk_token:
   if (file_system_parameters.concurrent_requests!=0)
   {
     ...
     {
       token = (int *)mallocame(sizeof(int));
     ...
     else
     {
       ...
       switch (copy->IO_blocking_point)
       {
         ...
         case IO_Block_Collective:
           ...
           thread = copy->twin_thread;
           Paraver_thread_block_IO (0, IDENTIFIERS(copy),
                   copy->last_paraver, current_time);
           copy->last_paraver = current_time;
           ...
           node = get_node_of_thread (copy);
	   cpu = get_cpu_of_thread (copy);
           Paraver_thread_IO (cpu->unique_number, IDENTIFIERS(thread),
                          current_time, new_time);
           copy->last_paraver = new_time;
           break;
         default:
           panic("Unkown blocking IO point %d\n", copy->IO_blocking_point);       
       }
     }
   }
...
void
FS_init(char *filename, t_boolean hot)
{
   struct t_cpu   *cpu;
   struct t_node  *node;
   int i;
	...
	node = (struct t_node *) next_queue (&Node_queue))
   {
      ...
      {
	 cpu->io = (struct t_queue *) mallocame (sizeof (struct t_queue));
	 create_queue (cpu->io);
      }
      if (file_system_parameters.concurrent_requests!=0)
      {
	...
	Ptask = (struct t_Ptask *) next_queue (&Ptask_queue))
   {
     if (count_queue(&Ptask->MPI_IO_request_thread))
     {
	   ...
	   printf (": Warning FS end with %d threads waiting nonblock IO access in Ptask  %d\n",
		    count_queue (&(Ptask->MPI_IO_request_thread)), Ptask->Ptaskid);
...
new_io_operation (int operation_id, char *operation_name)
{
   if ((operation_id>=0) && (operation_id<=MAX_IO_OPERATIONS))
   {
     ...
     {
       panic ("Incorrect IO operation name %s for operation %d\nIt must be %s",
               operation_name, operation_id, Operation_Name[operation_id]);
     }
   }
   else
     panic ("Invalid operation identicator %d for %s\n", 
             operation_id, operation_name);
}

void
FS_parameters (double disk_latency, double disk_bandwidth, double block_size,
        int concurrent_requests, double hit_ratio)
{
