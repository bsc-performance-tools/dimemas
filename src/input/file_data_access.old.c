 ...
 *	PFC: NEW DIMEMAS DATA ACCES LAYER API IMPLEMENTATION
 *	REVISION 1.0
...
// Main struct, applications conteiner
// eneables mutil-application storage to api
...
struct t_queue* DAP_get_comm_granks(struct t_communicator* comm) {
...
void DAP_init_ptask(struct ptask_structure* ptask, int ptask_id, int task_num, int* th_num) {
...
void DAP_set_even_action(struct t_action* act, int actid, long int type, long int value) {
	act->action=actid;
...
struct t_action* DAP_create_action() {
...
struct t_action* DAP_create_next_action(struct t_action* act) {
	struct t_action* res = NULL;
	act->next = DAP_create_action();	
	if(act->next!=NULL) res = act->next;
	return res;
}


// Sets action atributes for message recive action
void DAP_set_action_msg_rcv(struct t_action* act, int actid, int task, int msgsize, int tag, int commid, int rcvt) {
	/* act->action = actid; */
	act->desc.recv.ori=task;
...
void DAP_set_action(struct t_action* act, int id, int task_id, int thread_id, double burst) {
	act->action=id;
	act->desc.compute.cpu_time = burst;
}

// Sets action atributes for message passing action type
void DAP_set_action_msg(struct t_action* act, int id, int dest, int msgs, int tag, int commid, int sync) {
	act->action = id;
	act->desc.send.mess_size=msgs;
	act->desc.send.dest=dest;
	act->desc.send.mess_tag=tag;
	act->desc.send.communic_id=commid;
	if(sync==0) { 
		act->desc.send.rendez_vous=0;
		act->desc.send.immediate=0;
	}
	if(sync==1) { 
		act->desc.send.rendez_vous=1;
		act->desc.send.immediate=0;

	}
	if(sync==2) { 
		act->desc.send.rendez_vous=0;
		act->desc.send.immediate=1;
...
void DAP_set_glob_action(struct t_action* act, int actid, int globopid, int commid, int rtask, 
		int rthread, long int bsent, long int brecv) {
	act->action = actid;
...
void DAP_report_error(char* message) {
	...
	if(main_struct.error_string==NULL) {
		main_struct.error_string=(char*)malloc(DATA_ACCESS_MAX_ERROR_LENGTH*sizeof(char));		
	} 
...
int DAP_sum_array(int* array, int size, int start, int end) {
	int i=0;
	int suma = 0;
	for(i=start;i<end;i++) {
		suma+=array[i];
...
double read_double(char* str, int length) {
	int i = 0;
	double res = 0;
	char simbol;
	int fin = 0;
	...
	while(i<length && fin==0) {
		simbol = str[i];
		if(simbol=='.' || simbol=='\0') fin = 1;
		else {
			cipher = convert_to_int(simbol);
			if(cipher==-1) return 0;
			bvalue = cipher;
			res = res*10 + bvalue;
...
*/
void DAP_set_sequence(long int* array, long int starting, int array_size) {
	int i=0;
	int value=starting;
...
*/
int DAP_add_to_array(long int* array, int array_size, long int element, int elements_in_array) {
	int res = 0;
...
*/
int DAP_extract_from_array(long int* array, long int* element, int elements_in_array) {
	int res = 0;
...
*/
int DAP_delete_arrays(long int** array, int array_size) {
	int i=0;
	if(array!=NULL) {
		for(i=0;i<array_size;i++) {
...
int DAP_create_arrays(long int** conteiner, int conteiner_size, int *array_size) {
	...
	for(i=0;i<conteiner_size;i++) {
		...
		for(j=0;j<array_size[i];j++) conteiner[i][j]=0;
		if(conteiner[i]==NULL) {
			return i;
			DAP_report_error("create_array in data_access_layer_api, failed to crate arrays.");
		}
	}
	return i;
}
...
*/
long int** DAP_create_array_of_arrays(int* array_size, int conteiner_size) {
	...
	if(conteiner!=NULL) {
		...
		if(n_arrays!=conteiner_size) DAP_delete_arrays(conteiner, n_arrays);
	}
	return conteiner;
}

// Checks if char type item is contained in array
int DAP_seek_char_item(char* array, int size, char item) {
	int res=-1;
	int i=0;
	int end=0;
...
int check_endoffile(FILE* stream) {
	int c;
	c=fgetc(stream);
	if(c==EOF) return 1;
	return 0;
...
*/
long int* get_array_conteiner(long int** conteiner, int conteiner_size, int array_index) {
	long int* item = NULL;
...
*/
long int get_array_item(long int* array, int array_size, int item_index) {
	long int item = -1;
...
*/
int set_array_item(long int* array, int array_size, int item_index, long int item_value) {
	int res=0;
	if(item_index<array_size && array_size>=0) {
		...
		res=-1;
		DAP_report_error("set_array_item in data_access_layer_api, no such record.");
	}
	return res;
}
...
/*
	...
	item_value: is a value of item to be assigned
*/
int set_conteiner_item(long int** conteiner, int conteiner_size, int array_id, int array_size, int item_id, long int item_value) {
	int result=-1;
	long int* array=NULL;
...
*/
long int get_item_conteiner(long int** conteiner, int conteiner_size, int array_id, int array_size, int item_id) {
...
int DAP_has_thread(struct app_struct* app, int task, int thread) {
	int num_tasks=0;
	int num_th=0;
	num_tasks=app->task_num;
	if(num_tasks<=task || task<0) return 0;
	num_th=app->thread_num[task];
	if(num_th<=thread || thread<0) return 0;
	return 1;
}	

// Prints number of internal streams currently in use
void DAP_print_streams(struct app_struct* app) {
	printf("Internal streams in use:%d\n", app->num_streams-app->num_free_streams);
}

// Returns comm number
int DAP_get_comm_num(struct app_struct* app) {
	return app->comm_num;
}

// Returns communicators of application
struct t_queue* DAP_get_comms(struct app_struct* app) {
	return app->comms;
}

// Prints communicators
void DAP_print_comms(struct app_struct* app) {
	char* item = NULL;
...
void DAP_print_app_id(struct app_struct* app) {
...
int DAP_get_off_v_app(struct app_struct* app) {
...
void DAP_set_o_length(struct app_struct* app, int length) {
	app->offset_length=length;
}

// Gets gets offset length in bytes
long int DAP_get_o_size(struct app_struct* app) {
	return app->offset_length;
}

// Print offset size in chars
void DAP_print_o_size(struct app_struct* app) {
	printf("Offset size in chars is %d\n", DAP_get_o_size(app));
}

// Gets offset type
int DAP_get_o_type(struct app_struct* app) {
	return app->offset_type;
}

// Prints offset type to standard output
void DAP_print_o_type(struct app_struct* app) {
	printf("Offset type is %d\n", DAP_get_o_type(app));
}

// Initalizes the application, should use this function to create applications
int DAP_initialize_app(struct app_struct* app, int ptask_id, int task_n, int* thread_n, int comm_n, int offset_t, long int offset_v, char* file_name, int stream_n, int h_length) {
	// Local variables
	...
	if(app->tasks_offsets==NULL || app->def_stream==NULL || app->comms==NULL) {
		
		DAP_report_error("DAP_initialize_app in data_access_layer_api, offsets lists failed to be created or default stream failed.");
		return -1;
	}
	...
	if(app->tasks_offsets==NULL) {
		...
		free(app->comms);
		DAP_report_error("DAP_initialize_app in data_access_layer_api, stream lists failed to be created.");
		return -1;
	}
	...
	if(app->streams==NULL) {
		...
		fclose(app->def_stream);
		free(app->comms);
		DAP_report_error("DAP_initialize_app in data_access_layer_api, streams failed to be created.");
		return -1;
	}
	for(i=0;i<stream_n;i++) app->streams[i]=NULL;
...
void DAP_end_app(struct app_struct* app) {
	int i=0;
	t_boolean res=0;
	// Frees offset list
...
int DAP_set_thread_stream(struct app_struct* app, int task_id, int thread_id, long int stream_id) {
	int result=-1; // Result variable, if less then 0 indicates error
	
	result = set_conteiner_item(app->tasks_stream, app->task_num, task_id, app->thread_num[task_id], thread_id, stream_id);
	return result;

}

// Sets thread offset value
int DAP_set_offset_thread(struct app_struct* app, int task_id, int thread_id, long int offset) {
	// Local variables
	int result=-1; // Result variable, if less then 0 indicates error
		
	result = set_conteiner_item(app->tasks_offsets, app->task_num, task_id, app->thread_num[task_id], thread_id, offset);
	return result;
}

// Gets thread stream index, this is used for eficient file reading
long int DAP_get_stream_thread(struct app_struct* app, int task_id, int thread_id) {
	long int stream=-1;
	stream = get_item_conteiner(app->tasks_stream, app->task_num, task_id, app->thread_num[task_id], thread_id);
	return stream;
}

// Gets thread offset value
long int DAP_get_offset_thread(struct app_struct* app, int task_id, int thread_id) {
	long int offset=-1;
	offset = get_item_conteiner(app->tasks_offsets, app->task_num, task_id, app->thread_num[task_id], thread_id);
	return offset;
}

// Prints all threads offset values
void DAP_print_offsets(struct app_struct* app) {
	int i=0;
	int j=0;
	for(i=1;i<DAP_get_task_num(app)+1;i++) {
		for(j=1;j<DAP_get_threads_num_task(app, i-1)+1;j++) {
			printf("Task %d and thread  %d has offset %d\n", i, j, DAP_get_offset_thread(app, i-1, j-1));
		}
	}
}
...
char* get_file_name(struct app_struct* app) {
	char* res=NULL;
	if(app==NULL) {
		res=NULL;
	} else {
...
int DAP_close_thread(struct app_struct* app, int task_id, int thread_id) {
	int res = 0;
	int assig_stream = 0;
	...
	if(assig_stream!=DATA_ACCESS_no_stream_assig) {
		res = DAP_add_to_array(app->free_streams, app->num_streams, assig_stream, app->num_free_streams);
...
FILE* DAP_get_action_stream(struct app_struct* app, int task_id, int thread_id) { 
	int res = 0;
	FILE* stream=NULL;
	...
	if(assig_stream==DATA_ACCESS_no_stream_assig) {
		...
		num_streams = DAP_extract_from_array(app->free_streams, &assig_stream, app->num_free_streams);
		...
		if(num_streams==-1) { 
			...
			if(stream!=NULL) {
				...
				app->num_free_streams = DAP_add_to_array(app->free_streams, app->num_streams, 
...
void DAP_set_ptask(struct app_struct* app, struct ptask_structure* ptask) {
	DAP_init_ptask(ptask, DAP_get_ptask_id(app), DAP_get_task_num(app), DAP_get_threads_num(app));
}
...
void DAP_start_backdoor_filter() {
	char open_mode = 'w';
	...
	if(op_log_stream==NULL) {
		...
		DAP_report_error("backdoor log filter could not be created in data_access_layer_api.");
	}
}

// Writes log to file
void DAP_write_op_log(int ptask_id, int task_id, int thread_id) {
	if(op_log_stream!=NULL) {
		fprintf(op_log_stream, "Application %d reads task %d  and thread %d action record.\n", ptask_id, task_id, thread_id);
	}
}

void DAP_close_op_backdoor_filter() {
	if(op_log_stream!=NULL) {
...
int create_data_access_layer() {
	int res = 0;
	int i=0;
	main_struct.apps==NULL;
	main_struct.error_string==NULL;
	main_struct.apps = (struct app_struct**)malloc(DATA_ACCESS_MAX_NUM_APP*sizeof(struct app_struct*));
	if(main_struct.apps==NULL) {
		DAP_report_error("create_data_access_layer in data_access_layer_api, not enough memeory.");
		return -1;
	}
	// Inialize apps to avoid problems
	for(i=0;i<DATA_ACCESS_MAX_NUM_APP;i++) main_struct.apps[i]=NULL;
...
int read_r(FILE* stream, char* buffer, int buffer_size, int offset, 
		char END_REG) {
	// Local variables
	...
	if(c==END_REG && i+1<buffer_size) {
		...
		if(char_buff==EOF && offset==i) {
			...
			if(i+1==buffer_size) chars_read = -1;
			else {
...
*/
int read_r_state(FILE* stream, char* buffer, int buffer_size, char END_REG, 
...
// Coping of buffers
int copy_buffer(char* from, int from_size, char* to, int to_size) {
	int res = 0;
	int i = 0;
...
int get_new_buff_size(int ini_size) {
...
char* increce_buffer(char* buffer, int buffer_size, int max_buffer_size, int* new_buffer_size) {
...
*/
char* read_r_autoinc(FILE* stream, int max_buffer_size, char END_REG, int offset, int* chars_r) {
	...
	char_r = read_r_state(stream, buffer, buffer_size, END_REG, offset, &state);		
	while(state==-2) {
		// Increment buffer here
		buffptr = increce_buffer(buffer, buffer_size, max_buffer_size, &new_buffer_size);
		if(buffptr==NULL) {
			free(buffer);
			buffer = NULL;
		} else {
...
*/
char* DAP_read_register_autoincr(FILE* stream, int max_buffer_size, int* chars_r) {
	char* res = NULL;
	int offset = 0;
	res = read_r_autoinc(stream, max_buffer_size, DATA_ACCESS_EOR, offset, chars_r);
	return res;
}

// Reading of field from buffer DEPRECATED
int DAP_read_f(char* buffer, int buffer_size, char* field, int field_size, int offset, char* F_DEL, int n_fd) {
	int c_read = -1;
	int i = offset;
	int j = 0;
	...
	if(DAP_seek_char_item(F_DEL, n_fd, buffer[i])==0
				&& j+1<field_size) 
	{
...
*/
int read_df(char* buffer, int buffer_size, char* field, 
...
int DAP_read_register(FILE* stream, char* buffer, int buffer_size) {
	// Local variables
	...
	if(chars_read==-1) {
		...
		if(chars_read==-2) {
			...
			DAP_report_error("DAP_read_register in data access api, buffer overfflow");			
		}
	}
	return chars_read;
}
...
*/
int DAP_read_field(char* buffer, int buffer_size, char* field, int field_size, int offset) {
	int c_read = 0;
	c_read = read_df(buffer, buffer_size, field, field_size, offset, FIELD_DELIMITERS, NUM_FIELD_DELIMITERS);
	if(c_read==-1) {	
...
*/
int DAP_read_delimited_field(char* buffer, int buffer_size, char* field, 
		int field_size, int offset, char* delimiters, int delim_size) {
	int c_read = 0;
	...
	c_read = read_df(buffer, buffer_size, field, field_size, offset, delimiters, delim_size);
	if(c_read==-1) {
		DAP_report_error("DAP_read_delimited_field in data_access_layer_api, failed.");
	}
	return c_read;
}

// Verifies if there is certan number of int fields in buffer, returns the offset of last delimiting char
int DAP_verify_num_int_fields(char* buffer, int buffer_size, int offset, int fields_num) {
	...
	for(i=0;i<fields_num;i++) {
		char_r=DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &value);
		if(char_r==-1) {
			res = -1;
		}
		offset=char_r;
...
int DAP_verify_bracket_sintx(char* buffer, int buffer_size, int offset) {
	...
	if(buffer[offset]!='(') {
		DAP_report_error("DAP_verify_bracket_sintx in data_access_layer_api, sintax error");
		return -1;
	}
	...
	if(field!=NULL) {
		deli_offset = DAP_read_delimited_field(buffer, buffer_size, field, field_size, offset, delim, delim_size);
		if(deli_offset==-1) { res = -1; }
		else { res=deli_offset; }
	} else {
		// Not enought memory
		DAP_report_error("DAP_verify_bracket_sintx in data_access_layer_api, not enough memory");
		res=-1;
	}
	free(field);
	return res;
...
*/
int DAP_double_field_read(char* buffer, int buffer_size, char* field, int field_size, int offset, double* field_value) {
	int res=-1;
	int c_read=0;
	double field_v=0;
...
*/
int long DAP_long_int_field_read(char* buffer, int buffer_size, char* field, int field_size, int offset, long int* offset_rec) {
	int res=-1;
	int c_read=0;
	long int off_r=0;
	c_read=DAP_read_field(buffer, buffer_size, field, field_size, offset);
...
*/
int DAP_int_field_read(char* buffer, int buffer_size, char* field, int field_size, int offset, int* rec) {
	int res=-1;
	int c_read=0;
	int r=0;
	c_read=DAP_read_field(buffer, buffer_size, field, field_size, offset);
...
int DAP_h_r_filef(char* buffer, int buffer_size, char* field, int field_size, int offset) {
	int res=0;
	int c_read=0;
	c_read=DAP_read_field(buffer, buffer_size, field, field_size, offset);
	if(c_read<0) {
		...
		if(strcmp(DATA_ACCESS_DIMEMAS_HELLO_SIGN, field)!=0) {
			res=-1;
			DAP_report_error("DAP_h_r_filef in data access API, hello sign is not #DIMEMAS");
		} else {			
			res=offset+c_read;
		}
	}
...
int DAP_h_r_tr_name(char* buffer, int buffer_size, char* field, int field_size, int offset) {
	int res=0;
	int c_read=0;
	c_read=DAP_read_field(buffer, buffer_size, field, field_size, offset);
	if(c_read<0) {
...
int DAP_h_r_offset(char* buffer, int buffer_size, char* field, int field_size, int offset, int* offset_type, 
		long int* offset_value) 
{
	int res=-1;
	int c_read=0;
	...
	long int off_v=0;
	// Read type of offset
	c_read=DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &off_t);
	if(c_read!=-1) {
		res=c_read;
		*offset_type=off_t;
		if(off_t == OFFSET_PRESENT) {
			// Read offset register position
			c_read=DAP_long_int_field_read(buffer, buffer_size, field, field_size, c_read, &off_v);
			res=c_read;
			*offset_value=off_v;
		}
	}
...
int DAP_h_r_num_tasks(char* buffer, 
                      int buffer_size,
                      char* field, 
                      int field_size, 
                      int offset, 
...
int DAP_h_verify_num_th(char* buffer, int buffer_size, int offset, int task_num) {
	int res = 0;
	int brack_ch=0;
	int num_ch=0;
 	if(task_num!=0){ 
		...
		num_ch=DAP_verify_num_int_fields(buffer, buffer_size, offset, task_num);
		if(num_ch==-1) {
			return -1;
		}
		if(num_ch==brack_ch) {
...
int DAP_h_r_num_threads(char* buffer, int buffer_size, char* field, int field_size, int offset, int task_num, int* th_num) {
	int res=0;
	int i=0;
	int value=0;
	int char_r=0;
	if(DAP_h_verify_num_th(buffer, buffer_size, offset, task_num)==-1) return -1;
	if(task_num!=0) {
...
int DAP_h_r_comm_num(char* buffer, int buffer_size, char* field, int field_size, int offset, int* comm_num) {
	int res=0;
	int c_read=0;
	// Verify ',' char in place
	if(buffer[offset]!=',') { 
		DAP_report_error("DAP_h_r_comm_num in data_access_layer_api, sintax error.");
		return -1;
	}
	offset=offset+1; // Avoiding ',' char
	c_read=DAP_int_field_read(buffer, buffer_size, field, field_size, offset, comm_num);
	if(c_read<0) {
		return -1;
	}
	res=c_read;
...
int DAP_h_check_and_save(struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size
		, int ptask_id, char* file_name, int char_r) 
{
	int res=0;
	int c_read=0;
	int offset=0;
	...
	offset=c_read;
	c_read=DAP_h_r_offset(buffer, buffer_size, field, field_size, offset, &offset_type, &offset_value);
	if(c_read<0) {
		return -1;
	}
	offset=c_read;
	c_read=DAP_h_r_num_tasks(buffer, buffer_size, field, field_size, offset, &task_num);
	if(c_read<0) {
		return -1;
	}
	offset=c_read;
	...
	c_read=DAP_h_r_num_threads(buffer, buffer_size, field, field_size, offset, task_num, th_num);
	if(c_read<0) {
		return -1;
	}
	offset=c_read;
	c_read=DAP_h_r_comm_num(buffer, buffer_size, field, field_size, offset, &comm_num);
	if(c_read<0) {
		return -1;
	}
	if(c_read!=char_r) {
		DAP_report_error("DAP_h_check_and_save in data access layer api, sintax error.");
		return -1;
	}
	res = DAP_initialize_app(app, ptask_id, task_num, th_num, comm_num, offset_type, offset_value, file_name, stream_n, char_r);
	...
	if(buffer!=NULL && field!=NULL && stream!=NULL) 
	{
		...
		if(c_read<0) {
			...
			res = DAP_h_check_and_save(app, buffer, buffer_size,
					field, field_size, ptask_id, file_name, c_read);
...
int DAP_ch_offset_register(char* buffer, int buffer_size) {
	int res=0;
	// Check offset rec id
	if(buffer[0]!=DATA_ACCESS_OFFDEF || buffer[1]!=':') {
		DAP_report_error("DAP_ch_offset_register in data_access_layer_api, sintax error.");
		return -1;
	}
	return res;
}
...
int DAP_read_offset_thread_rec(struct app_struct* app, FILE* stream, char* buffer, int buffer_size,
		char* field, int field_size, int task_id, int offset, int read_r) {
	int res = 0;
	int num_threads=0;
	int i=0;
	int char_r = 0;
	...
	for(i=0;i<num_threads;i++) {
		char_r = DAP_long_int_field_read(buffer, buffer_size, field, field_size, offset, &value);
		if(char_r<=0 || char_r>read_r) {
			
			DAP_report_error("DAP_read_offset_thread_recs in data_access_layer_api, sintax error");		
			return -1;
		}
...
int DAP_read_offset_recs(struct app_struct* app, FILE* stream, char* buffer, int buffer_size,
		char* field, int field_size) 
{
	...
	int set_res=0;
	char* secbuffer = NULL;
	secbuffer = DAP_read_register_autoincr(stream, DATA_ACCESS_MAX_BUFFER_SIZE, &read_r);
	...
	for(i=0;i<num_task;i++) {
		char_r = DAP_read_offset_thread_rec(app, stream, secbuffer, read_r, 
				field, field_size, i, offset, read_r);
...
int DAP_read_from_end_offset(struct app_struct* app, FILE* stream, char* buffer, int buffer_size,
		char* field, int field_size) 
{
	int res = 0;
	int pos_res = 0;
	...
	if(pos_res<0) {
		DAP_report_error("DAP_read_from_end_offset in data_access_layer_api, error on file stream positioning");
		return -1;
	}
	read_res = DAP_read_offset_recs(app, stream, buffer, buffer_size, field, field_size);
	if(read_res<0) {
		return -1;
	}
	res = read_res;
...
int DAP_exame_register_seek_off(char* buffer, int buffer_size, char* field, int field_size, 
		struct app_struct* app, FILE* stream, int rchar) {
	int res = 0;
	int fchar = 0;
	int id = 0;
	int task = 0;
	...
	long int f_pos=0;
	// Read id of register
	fchar = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &id);
	if(fchar==-1) return -1;
	if(DAP_seek_int_item(DATA_ACCESS_SUPPORTED_ACTIONS, DATA_ACCESS_SUPPORTED_ACT_NUM, id)==0) {
		// Read task, thread
		offset = fchar;
		fchar = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &task);
		if(fchar==-1) return -1;
		offset = fchar;
		fchar = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &thread);
		if(fchar==-1) return -1;
		offset = fchar;
...
int DAP_get_comparation_info(char* buffer, int buffer_size, char* field, int field_size, int* task, int* thread) {
	int res = 0;
	int offset = 0;
	int regid = 0;
	// Read register id
	res = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &regid);
	if(res<0) return -1;
	offset = res;
	if(DAP_seek_int_item(DATA_ACCESS_SUPPORTED_ACTIONS, DATA_ACCESS_SUPPORTED_ACT_NUM, regid)==0) {
		// Read fields
		res = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, task);
		if(res<0) return -1;
		offset = res;
		res = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, thread);
		if(res<0) return -1;
		offset = res;
	} else {
		// Error
...
int DAP_exam_seek(FILE* stream, struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size,
		long int* offset, long int cut_point, int task, int thread, long int low_limit, long int high_limit) {
	int res = 0;
	int LOWER = -1;
	int BIGGER = 1;
	int EQUAL  = 0;
	...
	res = DAP_get_comparation_info(buffer, buffer_size, field, field_size, &first_task, &first_thread);
	if(res<0) return ERROR_CASE;
...
// Dicotomic seek 
int DAP_dicotomic_seek(FILE* stream, struct app_struct* app, long int file_size,
		char* buffer, int buffer_size, char* field, int field_size,
		long int* offset, int task, int thread) {
	...
	while(low_limit<cut_point && cut_point<high_limit && found==0) {
		...
		aval = DAP_exam_seek(stream, app, buffer, buffer_size, field, field_size, 
				&exam_point, cut_point, task, thread, low_limit, high_limit);
...
int DAP_advaced_offset_seek(FILE* stream, struct app_struct* app, long int file_size, 
		char* buffer, int buffer_size, char* field, int field_size) {
	int res = 0;
	int task_num = 0;
	int thread_num = 0;
	...
	for(i=1;i<task_num+1 && end==0;i++) {
		...
		for(j=1;j<thread_num+1 && end==0;j++) {
			dic_res = DAP_dicotomic_seek(stream, app, file_size, buffer, buffer_size, 
...
// Sequencial seek
//
int DAP_sec_seek_offsets(char* buffer, int buffer_size, char* field, int field_size, 
	...
	while(rchar!=-1 && rchar!=-2) {
		resex = DAP_exame_register_seek_off(buffer, buffer_size, field, field_size, app, stream, rchar);
		if(resex==-1) return -1;
		rchar = DAP_read_register(stream, buffer, buffer_size);
	}
	return res;
...
int DAP_seek_offsets(char* buffer, int buffer_size, char* field, int field_size, struct app_struct* app, FILE* stream) {
	int res = 0;
	...
	if(file_size<DATA_ACCESS_THRESHOLD) {
		res = DAP_sec_seek_offsets(buffer, buffer_size, field, field_size, app, stream);
	} else {
		// Advanced search method
		res = DAP_advaced_offset_seek(stream, app, file_size, buffer, buffer_size, field, field_size);
	}
	return res;
}
...
int read_offsets(struct app_struct* app) {
	...
	if(offset_t == OFFSET_NOT_PRESENT) {
		...
		// No offset record
		// Seek for offsets
		res_seek = DAP_seek_offsets(buffer, buffer_size, field, field_size, app, stream);
		if(res_seek==-1) {
			res=-1;
		}
	}
  ...
  */
	if(offset_t == OFFSET_PRESENT) {
		...
		end_off_r=DAP_read_from_end_offset(app, stream, buffer, buffer_size, field, field_size);
		if(end_off_r==-1) {
			res=-1;
		}
		res=end_off_r;
...
int read_tasks_comm(struct app_struct* app, char* buffer, int buffer_size, 
		char* field, int field_size, int offset, int task_num, struct t_communicator* comm) {
	int res = 0;
	int i = 0;
	int char_r=0;
	int* task_id=NULL;
	...
	while(i<task_num) {
		...
		char_r=DAP_int_field_read(buffer, buffer_size, field, field_size, offset, task_id);
...
int read_comm_regs(struct app_struct* app, char* buffer, int buffer_size, char* field, 
		int field_size, int offset) {
	...
	char_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &commid);
	if(char_r<0) {
		free(comm);
		return -1;
	}
	...
	offset=char_r;
	char_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &taskc);
	if(char_r<0) return -1;
	//printf("Number of task involved in commuicator id: %d is %d\n", commid, taskc);
	offset=char_r;
	char_r = read_tasks_comm(app, buffer, buffer_size, field, field_size, offset, taskc, comm);
	if(char_r<0) return -1;
	res=char_r;
	// Store to app
...
int switch_obj_type(struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size, int offset, int type) {
	int res = 0;
  ...
  switch(type)
  {
    case DEF_COMMUNICATOR:
      ...
      res = read_comm_regs(app, buffer, buffer_size, field, field_size, offset);
      break;
    case DEF_FILE:
      break;
    ...
    default:
      ...
      DAP_report_error("switch_obj_type in data access layer api, invalid object type definition");
      res = -1;
  }
...
int DAP_read_def_reg(struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size, FILE* stream) {
	...
	start_off = 2;
	char_r = DAP_int_field_read(buffer, buffer_size, field, field_size, start_off, &obj_type);
	if(char_r<0) return -1;
	//printf("Object def type is:%d\n", obj_type);
	offset=char_r;
	// Switch procedure of every object type threting
	char_r = switch_obj_type(app, buffer, buffer_size, field, field_size, offset, obj_type);
	offset=char_r;
	if(char_r<0) return -1;
	if(char_rr!=char_r) return -1;
...
int DAP_do_def_reading(struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size, FILE* stream) {
	int res = 1; // 1 is OK, 0 end of registers, -1 error
	while(res==1) {
		res = DAP_read_def_reg(app, buffer, buffer_size, field, field_size, stream);
	}
	if(res==-1) return -1;
	return 0;
}
...
int DAP_set_def_reading(struct app_struct* app, char* buffer, int buffer_size, char* field, int field_size, FILE* stream) { 
	int res=0;
	int char_r = 0;
	int off_t=0;
...
int DAP_read_definitions(struct app_struct* app) {
	int res = 0;
	FILE* stream=NULL;
	char open_mode='r';
	...
	int buffer_size=DATA_ACCESS_STANDARD_BUFFER_SIZE;
	int field_size=DATA_ACCESS_STANDARD_FIELD_SIZE;
	if(DAP_get_def_flag(app)==1) return res;
	//char* buffer=(char*)malloc(buffer_size*sizeof(char));
	//char* field=(char*)malloc(field_size*sizeof(char));
	char buffer[DATA_ACCESS_STANDARD_BUFFER_SIZE];
	char field[DATA_ACCESS_STANDARD_FIELD_SIZE];
	...
	if(buffer!=NULL && field!=NULL) {
		c_read = DAP_set_def_reading(app, buffer, buffer_size, field, field_size, stream);
		if(c_read==-1) res = -1;
	} else {
		DAP_report_error("DAP_read_definitions in data access api, not enought memory.");
		res = -1;
	}
	//free(buffer);
	//free(field);
...
int DAP_message_rcv_read(char* buffer, int buffer_size, char* field, int field_size, int task_id, int th_id, 
	...
	int rcvt=0;
	int fchar_r=0;
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &task);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &msgsize);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &tag);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &commid);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &rcvt);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
...
int DAP_message_pass_read(char* buffer, int buffer_size, char* field, int field_size, int task_id, int th_id, 
		struct t_action* act, int offset, int actid, int rchar_r) {
	int res = 0;
	// Read 
	// 1. dest_task_id
	...
	int fchar_r = 0;
	// Dest reading
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &dest);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	// Msg size reading
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &msgs);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	...
	offset = fchar_r;
	// Sync read
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &sync);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
...
int DAP_cpu_burst_read(char* buffer, int buffer_size, char* field, int field_size, int task_id, int th_id, 
		struct t_action* act, int offset, int actid, int rchar_r) {
	int fchar_r = 0;
	int res = 0;
	double burst = 0;
	
	fchar_r = DAP_double_field_read(buffer, buffer_size, field, field_size, offset, &burst);
	...
	if(fchar_r==rchar_r) {
		...
		res = -1;
		DAP_report_error("Action read register in data access api, sintax error.");
	}
	return res;
}
...
int DAP_even_read(char* buffer, int buffer_size, char* field, int field_size, int task_id, int th_id, 
		struct t_action* act, int offset, int actid, int rchar_r) {
	int res = 0;
	long int type = 0;
	long int value = 0;
	...
	while(go==1) {
		...
		fchar_r = DAP_long_int_field_read(buffer, buffer_size, field, field_size, offset, &type);
		if(fchar_r==-1) {
			DAP_report_error("Action read register in data access api, sintax error.");
			return -1;
		}
		...
		fchar_r = DAP_long_int_field_read(buffer, buffer_size, field, field_size, offset, &value);
		if(fchar_r==-1) {
			DAP_report_error("Action read register in data access api, sintax error.");
			return -1;
		}
		...
		if(fchar_r==rchar_r) {
			...
			if(fchar_r<rchar_r) {
				...
				if(next!=NULL) {
					...
					DAP_report_error("Action read register in data access api, memory error.");
					go=0;
					res = -1;
				}
			} else {
...
//
// returns: total size of register in chars/
int DAP_globop_read(char* buffer, int buffer_size, char* field, int field_size, int task_id, int th_id, 
		struct t_action* act, int offset, int actid, int rchar_r) {
	int res = 0;
	int fchar_r = 0;
	int gop_id = 0;
	...
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &gop_id);
	...
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &comm_id);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &r_task_id);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;

	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &r_thread_id);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;

	fchar_r = DAP_long_int_field_read(buffer, buffer_size, field, field_size, offset, &bsent);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	
	fchar_r = DAP_long_int_field_read(buffer, buffer_size, field, field_size, offset, &brecv);
	if(fchar_r==-1) {
		return -1;
	}
	offset = fchar_r;
	...
	if(fchar_r==rchar_r) {
		res = fchar_r;
		DAP_set_glob_action(act, actid, gop_id, comm_id, r_task_id, r_thread_id, bsent, brecv);
	} else {
		res = -1;
		DAP_report_error("Action read register in data access api, sintax error.");
	}
...
int DAP_read_actions_fields(char* buffer, int buffer_size, char* field, int field_size, 
		int task_id, int th_id, struct t_action* act, int offset, int actid, int rchar_r) {
	int res = 0;
	int fchar_r=0;
	int task=0;
	int thread=0;
	...
	// Read thread
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, offset, &thread);
	if(fchar_r==-1) {	
		return -1;
	}
	...
	if(task==task_id && thread==th_id) {
		...
		if(actid == RECORD_CPU_BURST) {
			// CPU BURST action
			// Read burst duration
			fchar_r = DAP_cpu_burst_read(buffer, buffer_size, field, field_size, task_id,
					th_id, act, offset, WORK, rchar_r);
			if(fchar_r==-1) {
				return -1;
			}
			res=0;
		...
		if(actid == RECORD_MSG_SEND) {
			...
			// 4. comm_id
			// 5. syncronism
			fchar_r = DAP_message_pass_read(buffer, buffer_size, field, field_size, task_id,
					th_id, act, offset, SEND, rchar_r);
			if(fchar_r==-1) return -1;
			res=0;
		}
		if(actid == RECORD_MSG_RECV) {
			// MESSAGE RECIVE action
			fchar_r = DAP_message_rcv_read(buffer, buffer_size, field, field_size, task_id,
					th_id, act, offset, RECV, rchar_r);
			if(fchar_r==-1) return -1;
			res=0;
		}
		if(actid == RECORD_EVENT) {
			// EVENT action 
			fchar_r = DAP_even_read(buffer, buffer_size, field, field_size, task_id,
		...
		if(actid == RECORD_GLOBAL_OP) {
			// GLOBAL OPERATION
			fchar_r = DAP_globop_read(buffer, buffer_size, field, field_size, task_id,
...
int DAP_read_action_reg(struct app_struct* app, int task_id, int thread_id, struct t_action* act, FILE* stream) {
	int res = 0;
	int char_r = 0;
	int fchar_r = 0;
	int jump_res = 0;
	...
	//char field[DATA_ACCESS_STANDARD_FIELD_SIZE];
	if(buffer==NULL) {
		DAP_report_error("DAP_read_action_reg in data api layer, not enought memory.");
		return -1;
	}
	...
	fchar_r = DAP_int_field_read(buffer, buffer_size, field, field_size, 0, &id);
	if(fchar_r==-1) {
		free(buffer);
		free(field);
		return -1;
	...
	if(DAP_seek_int_item(DATA_ACCESS_SUPPORTED_ACTIONS, DATA_ACCESS_SUPPORTED_ACT_NUM, id)==0) {
		...
		res = DAP_read_actions_fields(buffer, buffer_size, field, field_size, task_id, thread_id, act, fchar_r, id, char_r);
		// Save offset of task
		...
		if(res==0) {
			...
			if(res==-2) {
				...
				// Error, action register is not valid for thread or register is corruped, 
				res = DAP_close_thread(app, task_id, thread_id);
				res = -1;
			}
		}
...
int DAP_read_action(struct app_struct* app, int task_id, int thread_id, struct t_action* act) { 
	int res = 0;
	FILE* stream=NULL;
...
int DAP_initialize_app_proc(struct app_struct* app, int ptask_id, char* trace_file) {
	int res=0;
	int res_h_read=0;
	int res_o_read=0;
	int res_d_read=0;
	res_h_read = DAP_read_header(app, ptask_id, trace_file);
	if(res_h_read==-1) return -1;
	res_o_read = read_offsets(app);
	if(res_o_read==-1) return -1;
	res_d_read = DAP_read_definitions(app);
	if(res_d_read==-1) return -1;
	return res;
}
...
int DAP_add_app(int ptask_id, char* trace_file_location) {
	...
	if(num_a<DATA_ACCESS_MAX_NUM_APP) {
		main_struct.apps[num_a]=(struct app_struct*)malloc(sizeof(struct app_struct));
		if(main_struct.apps[num_a]==NULL) {
			DAP_report_error("DATA_ACCESS_get_error in data_access_layer_api, not enought memory");
			return -1;
		}
		res_i_proc=DAP_initialize_app_proc(main_struct.apps[num_a], ptask_id, trace_file_location);
		if(res_i_proc==-1) return -1;
...
DATA_ACCESS_init(int ptask_id, char* trace_file_location) {
	...
	if(main_struct.init_flag!=1) res_cr=create_data_access_layer();
	if(res_cr<0) return 0;
	res_i = DAP_add_app(ptask_id, trace_file_location);
	if(res_i==-1) return 0;
...
t_boolean DATA_ACCESS_get_ptask_structure(int ptask_id, struct ptask_structure** ptask_info) {
	t_boolean res=1;
	struct app_struct* app=NULL;
	struct ptask_structure* ptask = (struct ptask_structure*)malloc(sizeof(struct ptask_structure));
                            ...
                            struct t_action** action) 
{
  ...
  /* printf("Requested action for P%02d T%02d th%02d\n",
         ptask_id,
         task_id,
         thread_id); */
  ...
  if(app!=NULL && act!=NULL) {
		...
		res_act = DAP_read_action(app, task_id, thread_id, act);
		if(res_act==-1) {
			res = 0;
			*action=NULL;
			free(act);
		...
		res = 0;
		DAP_report_error("Invalid pask id.");
	}
	return res;
}
...
void DATA_ACCESS_end() {
	...
	for(i=0;i<main_struct.num_apps;i++) {
		if(main_struct.apps[i]!=NULL) DAP_end_app(main_struct.apps[i]);
	}
