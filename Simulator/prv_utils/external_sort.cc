...
void ExternalSort::Init()
{
  ...
  {
    die( "Unable to open temporal file %s: %s\n", TemporalFileNames.str().c_str(), IO_get_error() );
  }
...
void ExternalSort::NewRecord( int TYPE, int CPU, int Ptask, int Task, int Thread, prv_time_t timestamp, string ascii_record )
{
  if ( TYPE == PRV_EVENT )
  {
    /* Event records should be flush directly to a secondary file */
    CurrentEvent.FillRecord( TYPE, CPU, Ptask, Task, Thread, timestamp, ascii_record );
  ...
  else
  {
    ...
    Records[ CurrentRecord ].FillRecord( TYPE, CPU, Ptask, Task, Thread, timestamp, ascii_record );
    ++CurrentRecord;
  }
...
void ExternalSort::GenerateFinalTrace( FILE* DefinitiveTrace )
{
  ...
  Merge( ExternalSort::StatesAndCommsFileName, ExternalSort::EventsFileName, DefinitiveTrace );
...
void ExternalSort::SortStatesAndComms( void )
{
  ...
  if ( NumFiles == 1 )
  {
    ...
    if ( rename( ( ExternalSort::TmpFilesPrefix + ".0" ).c_str(), ( ExternalSort::TmpFilesPrefix ).c_str() ) == -1 )
    {
  ...
  else if ( NumFiles == 2 )
  {
    ...
    MergeFileName( ExternalSort::TmpFilesPrefix + ".0", ExternalSort::TmpFilesPrefix + ".1", ExternalSort::StatesAndCommsFileName );
  ...
  else
  {
    ...
    while ( NumFiles > 2 )
    {
      ...
      for ( k = 0, Count = 0; k < NumPairs; ++k )
      {
        ...
        mergeThreads.emplace_back( std::thread( &ExternalSort::MergeFileName, this, InFileName1, InFileName2, OutFileName ) );
      }

      for ( auto& th : mergeThreads )
        th.join();
      ...
      if ( 2 * NumPairs < NumFiles )
      {
        ...
        {
          die( "Unable to rename temporal sort files (%s to %s): %s\n", InFileName1.c_str(), OutFileName.c_str(), strerror( errno ) );
        }
      ...
      if ( NumFiles == 2 )
      {
        MergeFileName( ExternalSort::TmpFilesPrefix + "A.0", ExternalSort::TmpFilesPrefix + "A.1", ExternalSort::StatesAndCommsFileName );
...
void ExternalSort::FlushRecords( void )
{
  ...
  stable_sort( std::execution::par, Records.begin(), Records.begin() + CurrentRecord );
...
void ExternalSort::MergeFileName( string InFileName1, string InFileName2, string OutFileName )
{
...
void ExternalSort::Merge( string InFileName1, string InFileName2, FILE* OutFile )
{
#ifdef DEBUG
  cout << "Merging \"" << InFileName1 << " and \"" << InFileName2 << " to ";
...
void ExternalSort::Merge( FILE* InFile1, FILE* InFile2, FILE* OutFile, bool DefinitiveTrace )
{
...
size_t ExternalSort::FillBuffer( FILE* InFile, vector<SimpleParaverRecord>& Buffer, bool& MoreData )
{
  size_t k = 0;
...
bool ExternalSort::LoadRecord( FILE* InFile, SimpleParaverRecord& Record )
{
...
void ExternalSort::Copy( SimpleParaverRecord& Record, vector<SimpleParaverRecord>& Buffer, size_t& Index, FILE* OutFile, bool DefinitiveTrace )
{
