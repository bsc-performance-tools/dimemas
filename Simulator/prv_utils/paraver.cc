...
#define VERIFICA_GENERACIO_PARAVER \
  {                                \
    if ( !generar_paraver )        \
      return;                      \
  }
 ...
 * Private functions
 */
void NewState( int cpu, int ptask, int task, int thread, dimemas_timer init, dimemas_timer end, int state );

void NewEvent( int cpu, int ptask, int task, int thread, prv_time_t time, unsigned long long type, unsigned long long value );

void NewCommunication( int cpu_s,
                       int ptask_s,
                       int task_s,
                       int thread_s,
                       prv_time_t log_s,
...
class _EventCollapser
{
 ...
 public:
  ...
  prv_time_t timestamp( void )
  {
    return Timestamp;
  }

  bool isLastEvent( int CPU, int Ptask, int Task, int Thread, prv_time_t Timestamp )
  {
    if ( this->CPU == CPU && this->Ptask == Ptask && this->Task == Task && this->Thread == Thread && this->Timestamp == Timestamp )
    {
      return true;
    }
    return false;
  ...
  string toString( void )
  {
    ...
    {
      tmpToString << Events[ i ].first << ":" << Events[ i ].second;
    ...
    return tmpToString.str();
  }

  void reset( int CPU, int Ptask, int Task, int Thread, prv_time_t Timestamp )
  {
    this->CPU       = CPU;
    this->Ptask     = Ptask;
    this->Task      = Task;
 ...
 */
void PARAVER_Init( const char* output_trace, const char* pcf_insert, dimemas_timer start_time, dimemas_timer end_time, t_boolean priorities )
{
...
void PARAVER_Start_Op( int cpu, int ptask, int task, int thread, dimemas_timer time )
{
  prv_time_t time_prv;
...
void PARAVER_Idle( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_IDLE_ST );
}

void PARAVER_Running( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  {
    PRINT_TIMER( current_time );
    printf( ": Paraver Running State Printed\n\tOBJECT   CPU %d P%02d T%02d (t%02d)\n", cpu, ptask, task, thread );
  }

  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_RUNNING_ST );
}

void PARAVER_Startup( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_STARTUP_LAT_ST );
}

void PARAVER_Wait( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time, int type )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  {
    PRINT_TIMER( current_time );
    printf( ": Paraver Wait State Printed\n\tOBJECT   CPU %d P%02d T%02d (t%02d)\n", cpu, ptask, task, thread );
  }

  NewState( cpu, ptask, task, thread, init_time, end_time, type );
}

void PARAVER_Busy_Wait( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_BLOCKING_SEND_ST );
}

void PARAVER_Waiting_Links( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_WAITING_LNKS_ST );
}

/* TODO: This operation prints TEST/PROBE state... */
void PARAVER_Ctx_Switch( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_TEST_PROBE_ST );
}

void PARAVER_Data_Copy( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_DATA_COPY_ST );
}

void PARAVER_RTT( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_RTT_ST );
}

void PARAVER_IO_Op( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_IO_ST );
}

void PARAVER_IO_Blocked( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  {
    PRINT_TIMER( current_time );
    printf( ": Paraver IO Block Printed\n\tOBJECT   CPU %d P%02d T%02d (t%02d)\n", cpu, ptask, task, thread );
  }

  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_IO_ST );
}

void PARAVER_OS_Blocked( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
...
void PARAVER_Dead( int cpu, int ptask, int task, int thread, dimemas_timer time )
{
  ...
  NewState( cpu, ptask, task, thread, time, end_time, PRV_NOT_CREATED_ST );
}

void PARAVER_P2P_Comm( int cpu_s,
                       int ptask_s,
                       int task_s,
                       int thread_s,
                       dimemas_timer log_s,
                       ...
                       int tag )
{
  ...
  NewCommunication( cpu_s, ptask_s, task_s, thread_s, log_s_prv, phy_s_prv, cpu_r, ptask_r, task_r, thread_r, log_r_prv, phy_r_prv, size, tag );
}

void PARAVER_Event( int cpu, int ptask, int task, int thread, dimemas_timer time, unsigned long long type, unsigned long long value )
{
  prv_time_t time_prv;
  ...
  if ( type == IDLE_EVENT_TYPE || type == PRIORITY_SET_EVENT || type == PREEMPTION_SET_EVENT )
  {
    return;
  }
  ...
  NewEvent( cpu, ptask, task, thread, time_prv, type, value );
}

void PARAVER_Not_Created( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_NOT_CREATED_ST );
}

void PARAVER_Mem_Transf( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_MEMORY_TRNSF_ST );
}

void PARAVER_Others( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_OTHERS_ST );
}

void PARAVER_Thread_Sched( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_THREAD_SCHED_ST );
}

void PARAVER_Thread_Sync( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_SYNC_ST );
}

void PARAVER_Mem_Alloc( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time )
{
  VERIFICA_GENERACIO_PARAVER;
  ...
  NewState( cpu, ptask, task, thread, init_time, end_time, PRV_MEM_ALLOC_ST );
}
 ...
 ****************************************************************************/
void NewState( int cpu, int ptask, int task, int thread, dimemas_timer init_time, dimemas_timer end_time, int state )
{
  ...
  TraceMerger.NewRecord( StateType, cpu, ptask, task, thread, init_prv, to_ascii.str() );
}
                       ...
                       long int tag )
{
  ...
  TraceMerger.NewRecord( PRV_COMM, cpu_s, ptask_s, task_s, thread_s, timestamp, to_ascii.str() );
}
...
void GenerateParaverHeader( FILE* ParaverTraceFile )
{
  ...
  for ( ptask = (struct t_Ptask*)head_queue( &( Ptask_queue ) ); ptask != P_NIL; ptask = (struct t_Ptask*)next_queue( &( Ptask_queue ) ) )
  {
    for ( communicator = (struct t_communicator*)head_queue( &ptask->Communicator ); communicator != NULL;
