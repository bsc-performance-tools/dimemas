...
// GRAPH_print_state(from_taskid);
  ...
  struct dependency **str_deps =
    GRAPH_get_dependencies( from_taskid, GRAPH_TO_ALL, STRG_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL, &str_deps_size );
  ...
  switch ( action )
  {
    case SEND:
    {
      ...
      struct dependency *dep = GRAPH_get_dependency( to_taskid, from_taskid, STRG_DEP, RECV | WAIT, mess_tag, communic_id );

      int dep_set_size;
      struct dependency **deps = GRAPH_get_dependencies( to_taskid, from_taskid, SOFT_DEP, IRECV, mess_tag, communic_id, &dep_set_size );
    ...
    case WAIT:
    {
      ...
      struct dependency *deps = GRAPH_get_dependency( from_taskid, to_taskid, SOFT_DEP, SOFT_ACTION_RESOLVED, mess_tag, communic_id );

      // When an Isend is performed, it is directly an soft_action_resolved. WORKARROUND
      struct dependency *deps_aux = GRAPH_get_dependency( to_taskid, from_taskid, SOFT_DEP, SOFT_ACTION_RESOLVED, mess_tag, communic_id );
...
t_boolean DEADLOCK_manage_global_dependency( struct t_thread *thread )
{
  ...
  for ( int j = 0; j < size_comm; ++j )
  {
    ...
    struct dependency *dep = GRAPH_get_dependency( to_collective, from_collective, GLOP_DEP, from_action->action, TAG_ALL, COMMUNIC_ID_ALL );
...
void DEADLOCK_thread_finalized( struct t_thread *thread )
{
  ...
  for ( int i = 0; i < total_ranks; ++i )
  {
    ...
    dep_tome = GRAPH_get_dependencies( i, to_taskid, STRG_DEP | GLOP_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL, &dep_set_size );
...
t_boolean _is_deadlocked( int from )
{
  ...
  if ( res )
  {
    ...
    for ( i = 0; i < deepness; ++i )
    {
      ...
      {
        struct dependency *d;
        d = GRAPH_get_dependency( dep_chain_queue[ i ], dep_chain_queue[ i + 1 ], STRG_DEP | GLOP_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL );
...
int create_communicator( int *tasks_involved, int size )
{
  ...
  new_c->nodes_per_machine = (struct t_queue *)malloc( Simulator.number_machines * sizeof( struct t_queue ) );
...
int breakChainFrom( int *dep_chain_queue, int size )
{
  ...
  for ( i = 0; i < size - 1; i++ )
  {
    ...
    struct dependency *d = GRAPH_get_dependency( from_dep, to_dep, STRG_DEP | GLOP_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL );
    assert( d );
    switch ( d->action )
    {
      case SEND:
    ...
    while ( pch != NULL )
    {
      switch ( pos++ )
      {
        ...
        case 1:
          ...
          break;
        case 2:
          epr[ cnt ].recv_counter = atoi( pch );
          break;
        case 3:
          epr[ cnt ].irecv_counter = atoi( pch );
          break;
        case 4:
        ...
        case 5:
          ...
          break;
        case 6:
          epr[ cnt ].pendent_i_Send = atoi( pch );
          break;
        case 7:
          epr[ cnt ].pendent_i_Recv = atoi( pch );
          break;
        case 8:
          epr[ cnt ].pendent_glop = atoi( pch );
          break;
        case 9:
          epr[ cnt ].pendent_wait = atoi( pch );
          break;
          break;
        default:
          assert( FALSE );
...
t_boolean DEADLOCK_check_end()
{
  ...
  for ( i = 0; i < total_ranks; ++i )
  {
    GRAPH_print_state( i );
    int pendent_size;
    struct dependency **pendent =
      GRAPH_get_dependencies( i, GRAPH_TO_ALL, STRG_DEP | SOFT_DEP | GLOP_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL, &pendent_size );
...
int _get_earlier_task_index( int *dep_chain_queue, int deepness, t_boolean ignore_glops )
{
  ...
  {
    struct dependency *_dep =
      GRAPH_get_dependency( dep_chain_queue[ i ], dep_chain_queue[ i + 1 ], STRG_DEP | GLOP_DEP, GRAPH_ALL_ACTIONS, TAG_ALL, COMMUNIC_ID_ALL );
    if ( earlier_index == -1 )
    {
      earlier       = _dep->time;
      earlier_index = i;
