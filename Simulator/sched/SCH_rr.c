...
t_nano RR_get_execution_time( struct t_thread *thread )
{
  ...
  {
    ex_time = MIN( machine->scheduler.quantum - sch_rr->last_quantum, action->desc.compute.cpu_time );
    sch_rr->last_quantum += ex_time;
  }
...
struct t_thread *RR_next_thread_to_run( struct t_node *node )
{
  return ( (struct t_thread *)outFIFO_queue( &( node->ready ) ) );
}

void RR_init_scheduler_parameters( struct t_thread *thread )
{
  struct t_SCH_rr *sch_rr;
...
void RR_clear_parameters( struct t_thread *thread )
{
  struct t_SCH_rr *sch_rr;
...
void RR_init( char *filename, struct t_machine *machine )
{
  FILE *fp;
  int j;
  char buf[ 256 ];
...
void RR_copy_parameters( struct t_thread *th_o, struct t_thread *th_d )
{
...
void RR_free_parameters( struct t_thread *thread )
{
  struct t_SCH_rr *sch_rr;
