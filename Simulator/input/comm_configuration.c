...
t_boolean load_machine_comm_policy(int       machine_id,
                                   t_boolean all_machines,
                                   char     *policy_str);

t_boolean load_scheduling_quantum(int        machine_id,
                                  t_boolean  all_machines,
                                  int        quantum);
...
void COMM_CONFIGURATION_Load_General_Comms_Definition(char* comm_conf_filename,
                                                      FILE* comm_conf_file)
{
  ssize_t read;
  ...
  while ( (read = getline(&current_line, &lenght, comm_conf_file)) != -1)
  {
    ...
    if (matches == 2)
    {
      ...
      if (load_machine_comm_policy(machine_id,
                                   apply_to_all_machines,
                                   mini_buffer) == FALSE)
      {
        die("Error loading machine policy on (%s:%d): %s",
            comm_conf_filename,
            line,
            comm_conf_error_message);
      }
    }
    ...
    if (matches == 1 && apply_to_all_machines == TRUE)
    {
      ...
      if (load_machine_comm_policy(machine_id,
                                   ...
                                   mini_buffer) == FALSE)
      {
        die("Error loading machine policy (%s:%d): %s",
            comm_conf_filename,
            line,
            comm_conf_error_message);
      }
    ...
    {
      die("Error loading communications configuration (%s:%d): expected quantum size",
          comm_conf_filename,
          line);
    }
    else if (matches == 1 && expected_quantum_definition == TRUE)
    {
      if (!load_scheduling_quantum(machine_id,
                                   apply_to_all_machines,
                                   quantum))
      {
        die("Error loading scheduling quantum (%s:%d): %s",
            comm_conf_filename,
            line,
            comm_conf_error_message);
      }
    ...
    if (matches == 2)
    {
      ...
      {
        die("Error loading machine flight times (%s:%d): %s",
            comm_conf_filename,
            line,
            comm_conf_error_message);
      }
    ...
    if (matches == 6)
    {
      ...
      if (!load_global_op_parameters (FALSE, // NO external network
                                      ...
                                      FOUT_size))
      {
        die ("Error loading machine %d global operation definition (%s:%d): %s",
             machine_id,
             comm_conf_filename,
             line,
             comm_conf_error_message);
    ...
    if (matches == 5)
    {
      ...
      if (!load_global_op_parameters (TRUE, // External network
                                      ...
                                      FOUT_size))
      {
        die ("Error loading external network global operation definition (%s:%d): %s",
             comm_conf_filename,
             line,
             comm_conf_error_message);
      }
    ...
    {
      /* External network */
      if (!load_global_op_parameters (TRUE,
                                      machine_id,
                                      global_OP,
                                      FIN_model,
                                      FIN_size,
      ...
      {
        /* Rest of machines */
        if (!load_global_op_parameters (FALSE,
                                        machine_id,
                                        global_OP,
                                        FIN_model,
                                        FIN_size,
                                   ...
                                   char     *policy_str)
{
      ...
      switch (scheduling_types)
      {
        ...
        default:
          sprintf(comm_conf_error_message,
                  "Communication policy \"%s\" not implemented yet",
                  policy_str);
          return FALSE;
      }

      for (machines_it = 0;
           machines_it < Simulator.number_machines;
           machines_it++)
      {
  ...
  sprintf(comm_conf_error_message,
          "invalid communication policy name %s",
          policy_str);
                                    ...
                                    char *flight_times_str)
{
  ...
  while(flight_value_str != NULL)
  {
    ...
    {
      fprintf(stderr,
             "   * WARNING: Too many flight times for machine %d\n",
             machine_id);
      break;
    }
  ...
  {
    sprintf(comm_conf_error_message,
            "expected %d flight times and just %d present",
