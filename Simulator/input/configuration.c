...
void CONFIGURATION_load_mapping( char *tracefile, int *tasks_mapping, int tasks_mapping_length );

t_boolean CONFIGURATION_load_tasks_mapping_array( char *tracefile, int *tasks_mapping, int tasks_mapping_length );
...
void CONFIGURATION_New_Definition( struct t_queue *definition_fields, struct t_entry *definition_structure )
{
  struct t_field *f;
  ...
  for ( f = (struct t_field *)outFIFO_queue( definition_fields ); f != (struct t_field *)0; f = (struct t_field *)outFIFO_queue( definition_fields ) )
  {
    if ( f->tipo == 3 )
    {
      struct t_field *f2;

      for ( f2 = (struct t_field *)outFIFO_queue( f->value.arr.q ); f2 != (struct t_field *)0;
...
void new_node_info( struct t_queue *q, struct t_entry *en )
{
  ...
  int no_input, no_output;
  double local_startup, remote_startup;
  double relative;
  double local_bandwidth;
  double external_net_startup;
  double local_port_startup, remote_port_startup;
  double local_memory_startup, remote_memory_startup;
...
void new_map_info( struct t_queue *q, struct t_entry *en )
{
  ...
  for ( j = 0, f2 = (struct t_field *)head_queue( f->value.arr.q ); j < tasks_mapping_length;
        j++, f2   = (struct t_field *)next_queue( f->value.arr.q ) )
  {
    ...
    {
      near_line();
      die( "Node %d does not exist", current_node_id );
    }
...
void new_conf_files( struct t_queue *q, struct t_entry *en )
{
  ...
  if ( strlen( f->value.string ) != 0 )
  {
    ...
    if ( ( comm_conf_file = IO_fopen( comm_conf_filename, "r" ) ) == NULL )
    {
      die( "Unable to set and open communications configuration file (%s): %s\n", comm_conf_filename, IO_get_error() );
    }
  }
  else
  {
...
void new_mod_info( struct t_queue *q, struct t_entry *en )
{
  ...
  if ( f->value.real < 0 )
  {
    ...
    TASK_module_new_duration( module_type, module_value, const_burst_duration );
...
void new_d_conn( struct t_queue *q, struct t_entry *en )
{
  ...
  {
    near_line();
    die( "Dedicated connection %d not previously defined", d_conn_id );
  }
  ...
  for ( i = 0, f2 = (struct t_field *)head_queue( f->value.arr.q ); i < tags_size; i++, f2 = (struct t_field *)next_queue( f->value.arr.q ) )
  {
    tags[ i ] = f2->value.dec;
  }
  ...
  for ( i = 0, f2 = (struct t_field *)head_queue( f->value.arr.q ); i < comms_size; i++, f2 = (struct t_field *)next_queue( f->value.arr.q ) )
  {
...
void CONFIGURATION_load_mapping( char *tracefile, int *tasks_mapping, int tasks_mapping_length )
{
  ...
  if ( CONFIGURATION_mappings_read == 0 )
  {
    ...
    {
      TASK_New_Ptask_predefined_map( effective_tracefile, CONFIGURATION_parameter_predefined_map, CONFIGURATION_parameter_tasks_per_node );
    }
    else if ( tasks_mapping_length != 0 )
    {
      CONFIGURATION_load_tasks_mapping_array( effective_tracefile, tasks_mapping, tasks_mapping_length );
    }
    else
    {
      near_line();
  ...
  else
  {
    ...
    CONFIGURATION_load_tasks_mapping_array( tracefile, tasks_mapping, tasks_mapping_length );
  }
...
t_boolean CONFIGURATION_load_tasks_mapping_array( char *tracefile, int *tasks_mapping, int tasks_mapping_length )
{
  ...
  {
    die( "Unable to retrieve number of tasks of trace '%s': %s", tracefile, DATA_ACCESS_get_error() );
  }
  ...
  {
    near_line();
    die( "number of tasks in trace '%s' (%d) is larger than the tasks mapping (%d)", tracefile, tasks_count, tasks_mapping_length );
  }
