...
t_boolean parse_acc_nodes( char* current_record );

t_boolean NEW_CONFIGURATION_load_mapping( char* tracefile, int* tasks_mapping, int tasks_mapping_length, int predefined_map, int tasks_per_node );

t_boolean NEW_CONFIGURATION_load_tasks_mapping_array( char* tracefile, int* tasks_mapping, int tasks_mapping_length );
...
t_boolean parse_record( char* current_record )
{
  ...
  generate_error( &error_message, "Unknown configuration record type at line %d", current_line );
...
t_boolean parse_wan_info( char* record_fields )
{
  ...
  if ( matches == 7 )
  {
    if ( !SIMULATOR_set_wan_definition( erase_quotations( trim_string( wan_name ) ),
                                        ...
                                        communication_group_model ) )
    {
      generate_error( &error_message, "Wrong wide area network information record values at line %d (%s)", current_line, SIMULATOR_get_last_error() );
  ...
  {
    printf( "MATCHES = %d \n", matches );
    generate_error( &error_message, "Wrong wide area network information record structure at line %d", current_line );
...
t_boolean parse_env_info( char* record_fields )
{
  ...
  if ( matches == 7 )
  {
    ...
    if ( !SIMULATOR_set_machine_definition( machine_id,
                                            ...
                                            communication_group_model ) )
    {
      generate_error( &error_message, "Wrong environment information record values at line %d", current_line );
  ...
  {
    generate_error( &error_message, "Wrong enviroment information record field count at line %d", current_line );
...
t_boolean parse_node_info( char* record_fields )
{
  ...
  if ( matches == 14 )
  {
    ...
    if ( !SIMULATOR_set_node_definition( NO_NODE_ID,
                                         ...
                                         0 ) )
    {
      generate_error( &error_message, "Wrong node information record at line %d (%s)", current_line, SIMULATOR_get_last_error() );
  ...
  if ( matches == 11 )
  {
    ...
    if ( !SIMULATOR_set_node_definition( NO_NODE_ID,
                                         ...
                                         0 ) )
    {
      generate_error( &error_message, "wrong node information record values at line %d (%s)", current_line, SIMULATOR_get_last_error() );
      return FALSE;
    }
  ...
  {
    generate_error( &error_message, "wrong node information record field count at line %d", current_line );
...
t_boolean parse_multinode_info( char* record_fields )
{
  ...
  if ( matches == 14 )
  {
    ...
    if ( !SIMULATOR_set_multiple_node_definition( machine_id,
                                                  ...
                                                  0 ) )
    {
      generate_error( &error_message, "wrong multi node information record values at line %d (%s)", current_line, SIMULATOR_get_last_error() );
  ...
  {
    generate_error( &error_message, "wrong multi node information record field count at line %d", current_line );
...
t_boolean parse_map_info( char* record_fields )
{
  ...
  matches = sscanf( record_fields, "%[^,], %d, [%d] {%[^}]}};;\n", tracefile, &number_of_tasks, &array_length, mapping_tasks_to_nodes );
  ...
  if ( matches == 4 )
  {
    ...
    // Mapping tokening
    mapping_nodes_array = str_split( mapping_tasks_to_nodes, ',', &actual_array_length );

    if ( actual_array_length != array_length )
    {
      generate_error( &error_message, "wrong mapping information record values (incorrect map length) at line %d", current_line );
    ...
    for ( i = 0; i < actual_array_length; i++ )
    {
      ...
      {
        generate_error( &error_message, "wrong mapping information record value (node mapping) at line %d", current_line );
  ...
  {
    generate_error( &error_message, "Wrong mapping information record field count at line %d", current_line );

    return FALSE;
  }

  return NEW_CONFIGURATION_load_mapping( erase_quotations( trim_string( tracefile ) ), tasks_mapping, actual_array_length, MAP_NO_PREDEFINED, 0 );
}
...
t_boolean parse_predef_map_info( char* record_fields )
{
  ...
  matches = sscanf( record_fields, "%[^,]%*[, ]\"%[^\"]\"", tracefile, predefined_map );
  ...
  else if ( strstr( predefined_map, MAP_N_TASKS_PER_NODE_TXT ) != NULL )
  {
    ...
    {
      generate_error( &error_message, "wrong predefined mapping information record values (no tasks per node specified) at line %d", current_line );
    ...
    else
    {
      ...
      {
        generate_error( &error_message, "wrong predefined mapping information record (tasks per node value) at line %d", current_line );
...
*/
  ...
  return NEW_CONFIGURATION_load_mapping( erase_quotations( trim_string( tracefile ) ), (int*)0, 0, map_definition, tasks_per_node );
}
...
t_boolean parse_conf_files( char* record_fields )
{
  ...
  matches = sscanf( record_fields, "%[^,],%[^,],%[^,],%s", scheduler, file_system, communication, random );
  ...
  {
    generate_error( &error_message, "wrong configuration files record field count at line %d", current_line );
...
t_boolean parse_mod_info( char* record_fields )
{
  ...
  matches = sscanf( record_fields, "%d, %d, %lf", &type, &value, &execution_ratio );

  if ( matches == 3 )
  {
    if ( type < 0 )
    {
      generate_error( &error_message, "wrong modules files record value (negative type) at line %d", current_line );
    ...
    if ( value < 0 )
    {
      generate_error( &error_message, "wrong modules files record value (negative value) at line %d", current_line );
  ...
  {
    generate_error( &error_message, "wrong modules files record field count at line %d", current_line );
...
t_boolean parse_fs_params( char* record_fields )
{
  ...
  matches = sscanf( record_fields, "%lf, %lf, %lf, %d, %lf", &disk_latency, &disk_bandwidth, &block_size, &concurrent_requests, &hit_ratio );

  if ( matches == 5 )
  {
    if ( disk_latency < 0 )
    {
      generate_error( &error_message, "Wrong file system parameters record values (disk latency) at line %d", current_line );
    ...
    if ( disk_bandwidth < 0 )
    {
      generate_error( &error_message, "Wrong file system parameters record values (disk bandwidth) at line %d", current_line );
    ...
    if ( block_size < 0 )
    {
      generate_error( &error_message, "Wrong file system parameters record values (block size) at line %d", current_line );
    ...
    {
      generate_error( &error_message, "Wrong file system parameters record values (concurrent values) at line %d", current_line );
    ...
    {
      generate_error( &error_message, "Wrong file system parameters record values (hit ratio) at line %d", current_line );

      return FALSE;
    }

    FS_Parameters( disk_latency, disk_bandwidth, block_size, concurrent_requests, hit_ratio );
  }
  else
  {
    generate_error( &error_message, "Wrong file system parameters record field count at line %d", current_line );
...
t_boolean parse_d_conn( char* record_fields )
{
  ...
  if ( matches == 15 )
  {
    ...
    if ( !SIMULATOR_set_dedicated_connection_definition( connection_id,
                                                         ...
                                                         flight_time ) )
    {
      generate_error( &error_message, "Wrong dedicated connection information at line %d: %s", current_line, SIMULATOR_get_last_error() );
  ...
  {
    generate_error( &error_message, "Wrong dedicated connection information record field count at line %d", current_line );
...
{
  int matches, node_id, num_gpu_in_node, num_acc_buses;
  ...
  {
    generate_error( &error_message, "Wrong accelerator nodes information record field count at line %d", current_line );
...
t_boolean NEW_CONFIGURATION_load_mapping( char* tracefile, int* tasks_mapping, int tasks_mapping_length, int predefined_map, int tasks_per_node )
{
  ...
  if ( NEW_CONFIGURATION_mappings_read == 0 )
  {
    ...
    else
    {
      ...
      {
        generate_error( &error_message, "no tracefile provided, please use the '--dim' parameter to supply an application trace to simulate" );
        return FALSE;
      }
    }
    ...
    if ( NEW_CONFIGURATION_parameter_predefined_map != MAP_NO_PREDEFINED )
    {
      TASK_New_Ptask_predefined_map( effective_tracefile, NEW_CONFIGURATION_parameter_predefined_map, NEW_CONFIGURATION_parameter_tasks_per_node );
    }
    else if ( tasks_mapping_length != 0 )
    {
      if ( !NEW_CONFIGURATION_load_tasks_mapping_array( effective_tracefile, tasks_mapping, tasks_mapping_length ) )
      {
        return FALSE;
      }
    }
    ...
    {
      TASK_New_Ptask_predefined_map( effective_tracefile, predefined_map, tasks_per_node );
    }
    else
    {
      generate_error( &error_message, "inconsistent task mapping at line %d", current_line, DATA_ACCESS_get_error() );
      return FALSE;
    }
  }
  else
  {
    if ( tracefile == (char*)0 )
    {
      generate_error( &error_message, "no trace provided for mapping at line %d", current_line );
      return FALSE;
    }

    if ( tasks_mapping_length != 0 )
    {
      if ( !NEW_CONFIGURATION_load_tasks_mapping_array( tracefile, tasks_mapping, tasks_mapping_length ) )
      {
        return FALSE;
      }
    }
    ...
    {
      TASK_New_Ptask_predefined_map( tracefile, predefined_map, tasks_per_node );
    }
    else
    {
      generate_error( &error_message, "inconsistent task mapping at line %d", current_line, DATA_ACCESS_get_error() );
...
t_boolean NEW_CONFIGURATION_load_tasks_mapping_array( char* tracefile, int* tasks_mapping, int tasks_mapping_length )
{
  ...
  {
    generate_error( &error_message, "unable to retrieve number of tasks of trace '%s': %s", tracefile, DATA_ACCESS_get_error() );
    return FALSE;
  }
  ...
  if ( tasks_mapping_length >= tasks_count )
  {
    ...
    TASK_New_Ptask( tracefile, tasks_count, tasks_mapping );
  }
  else
  {
    generate_error( &error_message,
