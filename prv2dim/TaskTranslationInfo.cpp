  ...
  while (!feof (TemporaryFile))
    {
      EffectiveBytes = fread (Buffer,
			      sizeof (char), sizeof (Buffer), TemporaryFile);
 ...
 ****************************************************************************/
bool
TaskTranslationInfo::ReorderAndFlush (void)
{
  UINT32 i;
  ...
  if (!FilePointerAvailable)
      ...
      if ((TemporaryFile = fopen (TemporaryFileName, "a")) == NULL)
	{
	  ...
	  sprintf (CurrentError,
		   "Error opening temporary file to task %02d", TaskId);
...
/* ToDimemas (GENERIC) ********************************************************/
bool
TaskTranslationInfo::ToDimemas (ParaverRecord_t Record)
{
  ...
  else if ((CurrentComm = dynamic_cast < PartialCommunication_t > (Record)) !=
	   NULL)
    {
...
/* ToDimemas (EVENT) **********************************************************/
bool
TaskTranslationInfo::ToDimemas (Event_t CurrentEvent)
{
  ...
  if (!MPIEventEncoding_Is_MPIBlock ((INT64) Type) &&
      ...
      if (OngoingDeviceSync && AcceleratorThread != ACCELERATOR_NULL)
	{
      ...
      if (Dimemas_User_Event (TemporaryFile,
			      TaskId, ThreadId, (INT64) Type, Value) < 0)
	{
	  SetError (true);
	  SetErrorMessage ("error writing output trace", strerror (errno));
	  return false;
  ...
  if (CUDAEventEncoding_Is_CUDABlock (Type) &&
      ...
      if (CUDAEventEncoding_Is_BlockBegin (Value))
	{
	  ...
	  if (Dimemas_Block_Begin (TemporaryFile,
				   TaskId,
				   ThreadId, (INT64) Type, (INT64) Value) < 0)
	    {
	      SetError (true);
	      SetErrorMessage ("error writing output trace",
			       strerror (errno));
      ...
      else
	{
	  if (CUDABlockIdStack.size () == 0
	      ...
	      if (FirstCUDARead)
		{
		  ...
		  // trace
		  cout << "WARNING: unbalanced CUDA blocks on original trace"
		    << endl;
		  cout << "Task " << TaskId + 1 << " Thread: " << ThreadId +
		    1 << " ";
		...
		{
		  if (!GenerateGPUBurst
		      (TaskId, ThreadId, Timestamp, LastBlockEnd))
		    {
		      SetError (true);
		      SetErrorMessage ("error writing output trace",
				       strerror (errno));
...
#endif
	  if (AcceleratorThread == ACCELERATOR_KERNEL
	      && Value != OCL_FINISH_VAL && Type != OCL_KERNEL_NAME_EV)
	    {			/* Device threads must include a synchronization before they start
				   /* Idle block start */
	      if (Dimemas_Block_Begin (TemporaryFile,
				       TaskId, ThreadId,
      ...
      else
	{
	  if (OCLBlockIdStack.size () == 0
	      ...
	      if (FirstOCLRead)
		{
		  // There is a event closing without is opening in the middle of the
		  // trace
		  cout << "WARNING: unbalanced OCL blocks on original trace"
		    << endl;
		  cout << "Task " << TaskId + 1 << " Thread: " << ThreadId +
		    1 << " ";
		  cout << "Time " << Timestamp << endl;
		...
		{		/* Kernel side launch finish */
		  if (!GenerateGPUBurst
		      (TaskId, ThreadId, Timestamp, LastGPUBurstBlock))
		    {
		      SetError (true);
		      SetErrorMessage ("error writing output trace",
				       strerror (errno));
	  ...
	  if (FlushClusterStack)
	    {
	      if (Dimemas_Block_Begin (TemporaryFile,
				       TaskId,
				       ThreadId,
				       (INT64) ClusterBlockIdStack.back ().
				       first,
				       (INT64) ClusterBlockIdStack.back ().
				       second) < 0)
		{
...
bool
TaskTranslationInfo::ToDimemas (PartialCommunication_t CurrentComm)
{
  Block_t CurrentBlock;
  ...
  if (MPIBlockIdStack.size () > 0)
    {
      CurrentBlock = MPIBlockIdStack.back ();
      CurrentBlockValue =
	MPIEventEncoding_DimemasBlockId ((MPI_Event_Values) CurrentBlock.
					 second);
	...
	case BLOCK_ID_MPI_Recv:
	  ...
	  if (CurrentComm->GetType () == LOGICAL_RECV)
				   ...
				   CommId, Size, (INT64) Tag) < 0)
		{
	      ...
	      if ((*AllTranslationInfo)[PartnerTaskId][0]->
		  pendent_i_Send_counter == 0)
		this->pendent_i_Recv_counter++;
	      else
		(*AllTranslationInfo)[PartnerTaskId][0]->
		  pendent_i_Send_counter--;
	    }
...
/* Event2GlobalOp *************************************************************/
void
TaskTranslationInfo::Event2GlobalOp (Event_t CurrentEvent)
{
...
void
TaskTranslationInfo::FinalizeGlobalOp (void)
{
  /* if (GlobalOpFields == 4 || GlobalOpFields == 5) */
  ...
  fprintf (stdout,
	   ...
	   PartialGlobalOp->GetThreadId (), PartialGlobalOp->GetTimestamp ());
#endif
...
/* CheckIprobeCounters ********************************************************/
bool
TaskTranslationInfo::CheckIprobeCounters (Event_t CurrentEvent)
{
...
/* GenerateBurst **************************************************************/
bool
TaskTranslationInfo::GenerateBurst (INT32 TaskId,
  ...
  if (FirstPrint || !BurstCounterGeneration)
      ...
      for (i = 0; i < RecordStack.size (); i++)
	{
	  ...
	  if (CurrentEvent->GetFirstType () == BurstCounterType)
	    {
	      OnTraceTime =
		1.0 * CurrentEvent->GetFirstValue () * BurstCounterFactor;
...
bool
TaskTranslationInfo::PrintPseudoCommunicationEndpoint (INT32 CommType,
						       INT32 TaskId,
						       INT32 ThreadId,
						       INT32 PartnerTaskId,
  ...
  if (PartnerThreadId != -1)
    {
      if (Dimemas_User_Event (TemporaryFile,
			      TaskId,
			      ThreadId, (INT64) 11, PartnerThreadId) < 0)
	{
	  SetError (true);
	  SetErrorMessage ("error writing output trace", strerror (errno));
	  return false;
...
void
TaskTranslationInfo::PrintStack (void)
{
  int i;
...
/* GenerateBurst **************************************************************/
bool
TaskTranslationInfo::GenerateGPUBurst (INT32 TaskId,
				       INT32 ThreadId,
